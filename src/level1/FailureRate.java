...
public class FailureRate {
    ...
    public int[] solution(int N, int[] stages) {
        int[] answer = {};
        ArrayList list = (ArrayList) Arrays.stream(stages)
                .boxed()
                .collect(Collectors.toList());
        HashMap<Integer, Double> failStage =new HashMap<>();
        int size = list.size();
        ...
        for(int i=1;i<N+1;i++){
            ...
            if(cnt ==0){
                failStage.put(i, Double.valueOf(0));
            }else {
                failStage.put(i, (double) cnt / size);
                size = size - cnt;
            }
        }


        answer = failStage.keySet().stream()
                .sorted((o1, o2) -> Double.compare(failStage.get(o2),failStage.get(o1)))
